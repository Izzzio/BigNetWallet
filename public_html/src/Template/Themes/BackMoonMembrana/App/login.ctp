        ...
        <div class="sign-block">
            <form class="sign-in-form" action="<?= URL_PREFIX ?>/app/login" method="POST">
                ...
                <div class="input-block sbm-block">
                    <input class="btn-sbm" type="submit" id="signButton" value="<?= __('Sign in') ?>">
                </div>
...
<script>
    function CheckUSA() {
        var email = $(this).val();
        $('#signButton').addClass('disabledState');
        $.get('/custom/call/CheckUSA?email=' + email, function (data) {
            $('#signButton').removeClass('disabledState');
            if(JSON.parse(data)) {
                console.log('USA', JSON.parse(data));
                $('.sign-in-form')[0].action = 'https://usa-sale.membrana.io/app/login';
            } else {
                console.log('NOT USA', JSON.parse(data));
                $('.sign-in-form')[0].action = 'https://sale.membrana.io/app/login';
            }
        })
    }

    $('#email').keyup(CheckUSA);
    $('#email').change(CheckUSA);
    CheckUSA.apply($('#email'));
